...
const GpfWorkflowDetailsPage  = () => {
const { appType, id, empCode } = useParams();
...
console.log(editData[appId]);
   ...
   if (field === "amountofadvancerequested") {
      ...
      const outstanding =
        ...
        Number(
  detailsMap[appId]?.details?.amountofadvanceoutstanding
) || 0
...
const loadDetails = async () => {
    ...
    try {
  console.log("LOADDETAILS ENTERED");
   console.log("empCode =", empCode);
      const base =
        appType === "withdrawl"
          ? "/gpf-withdrawl"
          : "/gpf-advance";
          console.log("Calling:",`${base}/status/${empCode}`);
  ...
  await api.get(
    `${base}/status/${empCode}`
      ...
      const trailRes =
        await api.get(
          `${base}/trail/${selectedId}`
        );

      setTrailMap({
        [selectedId]: trailRes.data
      });
...
useEffect(() => {
  ...
  console.log("DETAIL EFFECT FIRED");
  console.log("selectedId =", selectedId);
  console.log("appType =", appType);
  console.log("empCode =", empCode);
  console.log("loadDetails =", typeof loadDetails);

  loadDetails();

}, [selectedId, appType, empCode]);
...
const empCode =
  detailsMap[selectedId]?.master?.empcode;
...
if (!selectedAction) {
  ...
  setVerifyMessage({
    ...
    subtitle: "Please select an action before processing.",
    type: "warning"
  });
...
{!(
  ...
  <button
    ...
    onClick={() => {
      ...
      if (verifyMessage?.type === "success-navigate") {
        navigate("/workflow");
      }
...
export default GpfWorkflowDetailsPage ;
